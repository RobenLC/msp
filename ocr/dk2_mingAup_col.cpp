...
#ifdef MAKELIB
int add_image_to_model_up( void * img_buf, int img_size, int state )
#else
...
//char * main(){
...
#ifdef ming_Aup
   rpt_out.open("cv_mingAup_1_100.txt");
   rpt_out2.open"num_det_mingAup_1_100.txt");
   //--try read "num_det_tw_xxxx.txt"
   //---- 以知冠字號(號碼) , 對答案用
   imgnum_in.open("num_det_mingAup_1_100b.txt");
#endif
 ...
 //---------2020/4/15  compare golden table
  ...
  //---- read golden 冠字號(字母)
  golden_in.open("mingAup_1_100_gray_none.txt");
  //--- 記錄錯誤
  rec_out.open("mingAup_1_100_errorlist.txt");
...
else if( state == 1)
{
...
#endif  // #ifdef MAKELIB
  {
    ...
    //--- for ubuntu
	 ...
	 char *infont = "/home/root/ming_upA/aspA_up_%d.bmp";
...
else if( state == 2)
{
...
#ifdef ming_Aup
  ifstream coll_in("mingAup_1_100_errorlist.txt");
#endif
  ...
  do {
      ...
      //---2020/05/19
       if (own_list[char_num] == 0) {
         ...
         //--------- choose image
				 ...
				 char addfont[100];
				 char *addfont_path = "./mingAup_set1/M%d.bmp";
				 ...
				 char *orgfont_path = "./font_col_a/st_%d.bmp";
				 char addfont[100];
				 char *addfont_path = "./mingAup_set2/M%d.bmp";
				 sprintf(orgfont, orgfont_path, st_num);
				 sprintf(addfont, addfont_path, char_num);
				 cout << "org = " << orgfont << endl;
				 cout << "dest = " << addfont << endl;
	 ...
	 CopyFile("./font_col_a/special_font/M17.bmp",
		 "./mingAup_set1/M17.bmp");

	 //--- mingAup_set2
	 CopyFile("./font_col_a/special_font/M2.bmp",
		 "./mingAup_set2/M2.bmp");
	 CopyFile("./font_col_a/special_font/M17.bmp",
		 "./mingAup_set2/M17.bmp");
