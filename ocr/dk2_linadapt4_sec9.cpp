...
#ifdef MAKELIB
static void *p_model_ntu=NULL;
int bkocr( void * img_buf, int img_size, char *out_buf, int out_buf_size )
#else
int main()
#endif
{
...
#ifdef MAKELIB
    if( p_model_ntu == NULL )
        p_model_ntu = (void*)load_model("cv_tw_model_linear.txt");
    model_ntu = (struct model *)p_model_ntu;
    if( model_ntu == NULL )
        return 1;
...
//    cout << rpt_out.str() << "size= " << resultlen;
    ...
    strncpy( out_buf, rpt_out.str().c_str(),
        (out_buf_size<resultlen)? out_buf_size : resultlen );
#else
  rpt_out.close();
#endif
