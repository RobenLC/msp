...
Final:
   ...
   : 2020/6/4   : 採用 font_hv model(cv_tw_model_hv.txt) , OCR
   : 2020/6/5   : for DK2, 垂直翻轉,do OCR only
   : 2020/6/19  : ocr ming_upAB, ming_dw_AB
**********************************************************/
...
#define MODEL_UPA     "upA_model_hv.txt"
#define MODEL_DWA     "dwA_model_hv.txt"
//#define MODEL_A     "cv_tw_model_linear.txt"
//#define MODEL_B     "cv_tw_model_linear_hv.txt"
...
static vector<int> current_layer_holes(vector<Vec4i> layers, int index) {
...
static bool struct_cmp_by_x_pos(font_order a, font_order b)
{
	return a.x_pos < b.x_pos;
}

static bool struct_cmp_by_x_pos_inv(font_order a, font_order b)
{
	return a.x_pos > b.x_pos;
}
...
static bool struct_cmp_blk_xy(bd_order a, bd_order b)
{
	if ((a.x_pos < b.x_pos))
		return true;
...
//#define tw_sec14   //--aspA_up_1 - aspA_up_100
#define tw_sec15   //--aspA_dw_1 - aspA_dw_100
...
#endif
{
  ...
  int orghog_descr_size = 108;
#ifdef MAKELIB
       // p_model = (void*)load_model("cv_tw_model_linear.txt");
       // p_model_ntu = (void*)load_model("cv_tw_model_hv.txt");
    model_ntu = (struct model *)p_model;
    if( model_ntu == NULL )
    {
        if( o_size>0 )
            *out_buf=0;
	...
	//model_ntu = load_model("cv_tw_model_hv.txt");
...
#endif
#ifdef tw_sec13
  ofstream rpt_out("cv_det_tw_1080_1213.txt");
#endif
#ifdef tw_sec14
	ofstream rpt_out("cv_mingAup_1_100.txt");
#endif
#ifdef tw_sec15
	ofstream rpt_out("cv_mingAdw_1_100.txt");
#endif
 ...
 //while (cin >> k)
  {
    ...
    //--- for ubuntu
    #ifdef ubuntu
	// char *infont = "/home/root/tw_base_hv/org%d_1.bmp";
...
#else
   // char *infont = "D:\\prj_arm\\2019_matlabtest\\code\\tw_base_hv\\org%d_1.bmp";
    char *infont = "F:\\scansnap-img\\ming_downA\\aspA_dw_%d.bmp";
#endif
    #endif
...
#ifdef MAKELIB
//    cout << rpt_out.str();
    int resultlen = rpt_out.str().length();
//    cout << rpt_out.str() << "size= " << resultlen;

    strncpy( out_buf, rpt_out.str().c_str(),
        (o_size<resultlen)? o_size : resultlen );
#else
  rpt_out.close();
#endif
