    ...
    if (rlt == WAIT) {
        ...
        switch (pro) {
            ...
            case PSTSM:
                //sprintf(str, "PSTSM\n"); 
                //print_f(mlogPool, "bullet", str); 
                if (tmpAns == ASPFAT_STATUS_INIT) { /* long jumg */
                    evt = REGF; 
                    next = PSWT;
...
static int fs52(struct mainRes_s *mrs, struct modersp_s *modersp) 
{
    ...
    if (pParBuf->dirBuffUsed) {
        ...
        if (curDir) {
            ...
            while (br) {
                if (!br->ch) {
                    secStr = (br->dfclstnum - 2) * psec->secPrClst;
