...
create_window() {
    ...
    egl_pixmap = wl_egl_pixmap_create(480, 360, 0);
    if (egl_pixmap == EGL_NO_SURFACE) {
	fprintf(stderr, "Can't create egl egl_pixmap\n");
	exit(1);
    } else {
	fprintf(stderr, "Created egl egl_pixmap\n");
    }

    //egl_surface = eglCreateWindowSurface(egl_display, egl_conf, egl_window, NULL);
    //egl_surface = eglCreatePbufferSurface(egl_display, egl_conf, NULL);
    egl_surface = eglCreatePixmapSurface(egl_display, egl_conf, egl_pixmap, NULL);
    if (egl_surface == EGL_NO_SURFACE) {
	fprintf(stderr, "Can't create egl surface\n");
	exit(1);
    } else {
	fprintf(stderr, "Created egl surface\n");
    }
