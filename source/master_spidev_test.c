...
static int ring_buf_init(struct shmem_s *pp)
{
    ...
    for (idx=0; idx <pp->slotn; idx++) {
        pp->urun[idx] = 0;
        pp->uget[idx] = 0;
        memset(pp->pp[idx], 0xa5, SPI_TRUNK_SZ);
...
static int ring_buf_prod_tag(struct shmem_s *pp, int usbrn, int thrshold)
{
    char str[128];
    int leadn = 0;
    int folwn = 0;
...
static int ring_buf_cons_u(struct shmem_s *pp, char **addr)
{
    ...
    msync(pp->urun, sizeof(int) * DATA_RX_SIZE, MS_SYNC);    
...
static int usb_gate(struct usbhost_s *ppup, struct usbhost_s *ppdn)
{
    ...
    uphstx = ppup->pushtx;
    printf("[GW] uphstx 0:%d 1:%d\n", uphstx[0], uphstx[1]);
    uphsrx = ppup->pushrx;
    printf("[GW] uphsrx 0:%d 1:%d\n", uphsrx[0], uphsrx[1]);
    updvtx = ppup->pgattx;
    printf("[GW] updvtx 0:%d 1:%d\n", updvtx[0], updvtx[1]);
    updvrx = ppup->pgatrx;
    printf("[GW] updvrx 0:%d 1:%d\n", updvrx[0], updvrx[1]);
    dnhstx = ppdn->pushtx;
    printf("[GW] dnhstx 0:%d 1:%d\n", dnhstx[0], dnhstx[1]);
    dnhsrx = ppdn->pushrx;
    printf("[GW] dnhsrx 0:%d 1:%d\n", dnhsrx[0], dnhsrx[1]);
    dndvtx = ppdn->pgattx;
    printf("[GW] dndvtx 0:%d 1:%d\n", dndvtx[0], dndvtx[1]);
    dndvrx = ppdn->pgatrx;
    printf("[GW] dndvrx 0:%d 1:%d\n", dndvrx[0], dndvrx[1]);
                        ...
                        if ((pllcmd[ins] & 0xc0) == 0xc0) {
                            if (!pubffh) {
                                ...
                                if ((pllcmd[ins] & 0xc0) == 0x40) {
                                    ...
                                    printf("[GW] WARNNING!!! unknow ch: 0x%.2x \n", pllcmd[ins]);
                                    break;
                                }

                                mindex = ((chindex[0] & 0x3f) << 6) | (chindex[1] & 0x3f);

                                printf("[GW] pll%d get midx: %d(0x%.2x:0x%.2x) buffo: 0x%.8x buffh: 0x%.8x\n", ins, mindex, chindex[0], chindex[1], pubffo, pubffh);
                                ...
                                if (!outbf) {
                                    pllcmd[ins] = 0xff;
                                    write(infd[ins], &pllcmd[ins], 1);
                                    printf("[GW] in%d id:%d put chr: %c(0x%.2x) no outbf\n", ins, infd[ins], pllcmd[ins], pllcmd[ins]);
                                    //pubffm = pubffo->ubnxt;                                        
                                    //pubffh = pubffm;
                                            ...
                                            if(pubffh == pubffo) {
                                                ...
                                                while (pubffm) {
                                                    if (pubffm->ubnxt == pubffo) {
                                                        ...
                                                        //printf("[GW] remove current buf addr: 0x%.8x\n", pubffo);
                                                        break;
                                                    }
                                                    pubffm = pubffm->ubnxt;
                                                }
                                    ...
                                    //write(infd[ins], &pllcmd[ins], 1);
                                    printf("[GW] WARNNING!!! in%d id:%d cur index: %d get index: %d NOT handle buffer\n", ins, infd[ins], pubffo->ubindex, mindex);
                                }
                            }
                        }
                            ...
                            if (latcmd[ins] == 'q') {
                                printf("[GW] already in multiple id:%d lat:%c pll:%c\n", ins, latcmd[ins], pllcmd[ins]);
                            } else if (latcmd[ins] == 'e') {
                                printf("[GW] already stop multiple id:%d lat:%c pll:%c\n", ins, latcmd[ins], pllcmd[ins]);

                                write(outfd[ins], &latcmd[ins], 1);
                                //printf("[GW] out%d id:%d put chr: %c(0x%.2x) \n", ins, outfd[ins], latcmd[ins], latcmd[ins]);
                            } else if (latcmd[ins] == 'R') {
                                printf("[GW] already stop multiple id:%d lat:%c pll:%c waiting \n", ins, latcmd[ins], pllcmd[ins]);
                            ...
                            if (latcmd[ins] == 'c') {
                                printf("[GW] already in multiple single id:%d lat:%c pll:%c\n", ins, latcmd[ins], pllcmd[ins]);
                            } else if (latcmd[ins] == 'e') {
                                printf("[GW] already stop multiple single id:%d lat:%c pll:%c\n", ins, latcmd[ins], pllcmd[ins]);
        ...
        while(1) {
            ...
            if ((ix % 4) == 0) {
                printf("\n%d: ", ix);
                            ...
                            write(outfd[ins], &pllcmd[ins], 1);
                            printf("[GW] id%d pipe%d put chr: %c(0x%.2x) \n", ins, outfd[ins], pllcmd[ins], pllcmd[ins]);
                        }
                        else {
                    ...
                    case 3:
                        ...
                        if (latcmd[ins] == 'b') {
                            ...
                            lens = ring_buf_cons_u(ringbf[ins], &addrs);                
                            while (lens <= 0) {
                                printf("[GW] cons ring buff ret: %d \n", lens);
                                usleep(1000);
                                lens = ring_buf_cons_u(ringbf[ins], &addrs);
                            }
                            ...
                            *add32d = (uint32_t)addrs;
                            ...
                            if (pllcmd[ins] == 'E') {
                                if ((matcmd[ins] == 'Q') ||(matcmd[ins] == 'D')) {
                                    if (pubffcd[ins]->ublastsize == 0) {
                                        ...
                                        //printf("[GW] in%d id:%d put chr: %c(0x%.2x) \n", ins, infd[ins], matcmd[ins-1], matcmd[ins-1]);
                                    
                                        lens = 0;                                    
                                    } else {
                                        printf("[GW] ring%d meta size: %d \n", ins, lens);
                            ...
                            if (ins == 3) {
                                if (latcmd[0] == 'Q') {
                                    ...
                                    //printf("[GW] out%d id:%d put chr: %c(0x%.2x) \n", 2, outfd[2], latcmd[2], latcmd[2]);
                                    write(outfd[0], &latcmd[0], 1);
                                    //printf("[GW] out%d id:%d put chr: %c(0x%.2x) \n", 0, outfd[0], latcmd[0], latcmd[0]);
                                }
                                else if (latcmd[0] == 'R') {
                                    latcmd[3] = 's';
                                    chq = 'q';
                            ...
                            printf("[GW] id:%d get poll status: 0x%.2x !!!\n", ins, pllinf);

                            pollfo[0] = 'J';
                            pollfo[1] = pllinf;
                            write(outfd[ins], pollfo, 2);
                            
                            printf("[GW] id:%d out%d put info: %c + 0x%.2x \n", ins, outfd[ins], pollfo[0], pollfo[1]);  
                        }
                        else {
                    ...
                    default:
                        write(outfd[ins], &pllcmd[ins], 1);
                        printf("[GW] out%d put chr: %c(0x%.2x) total:%d\n", ins, pllcmd[ins], pllcmd[ins], evcnt);
                        break;
                    }
                }
            }
        ...
        if (cmdchr == 0x01) { 
            ...
            while(1) {
                ...
                if (recvsz > 0) {
                    ...
                    usbfolw = ring_buf_prod_tag(pTx, usbrun, 500);
                    //printf("[%s] usbfolw: %d, usbrun: %d - m2\n", strpath, usbfolw, usbrun);
                }
...
static char spi1[] = "/dev/spidev32766.0"; 
    ...
    if (argc > 6) {
        ...
        int fsize = 0; 
        int buffsize = 0;
        buffsize = PT_BUF_SIZE;//128*1024*1024;
...
#define USB_HS_SAVE_RESULT_DV 0
    if (sel == 27){ /* usb printer test usb scam */
        ...
        char *ptfileSend=0;
        static char ptfileSendUsb[] = "/mnt/mmc2/usb/greenhill_05.jpg";
        FILE *fsend;
        struct pollfd ptfd[1];
...
struct sysinfo {
	...
	char _f[20-2*sizeof(__kernel_ulong_t)-sizeof(__u32)];	/* Padding: libc5 uses this.. */
        ...
        if(!fpd) {
            ptfileSend = ptfileSendUsb;
            printf(" open file [%s] \n", ptfileSend);
            fsend = fopen(ptfileSend, "r");
        } else {
            fsend = fpd;
            ptfileSend = data_path;
        }
        ...
        usbTx->urun = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
        usbTx->uget = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
#else
        ...
        usbTxd->urun = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
        usbTxd->uget = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
#else
        ...
        gateTx->urun = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
        gateTx->uget = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
#else
        ...
        gateTxd->urun = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
        gateTxd->uget = (int *)aspSalloc(sizeof(int) * DATA_RX_SIZE);
#else
        ...
        printf("[DV]  pipeTx 0:%d 1:%d\n", pipeTx[0], pipeTx[1]);
        pipe2(pipeRx, O_NONBLOCK);
        printf("[DV]  pipeRx 0:%d 1:%d\n", pipeRx[0], pipeRx[1]);
        
        pipe2(pipeTxd, O_NONBLOCK);
        printf("[DV]  pipeTxd 0:%d 1:%d\n", pipeTxd[0], pipeTxd[1]);
        pipe2(pipeRxd, O_NONBLOCK);
        printf("[DV]  pipeRxd 0:%d 1:%d\n", pipeRxd[0], pipeRxd[1]);
        
        pipe2(gateUpTx, O_NONBLOCK);
        printf("[DV]  gateUpTx 0:%d 1:%d\n", gateUpTx[0], gateUpTx[1]);        
        pipe2(gateUpRx, O_NONBLOCK);
        printf("[DV]  gateUpRx 0:%d 1:%d\n", gateUpRx[0], gateUpRx[1]);
        
        pipe2(gateDnTx, O_NONBLOCK);
        printf("[DV]  gateDnTx 0:%d 1:%d\n", gateDnTx[0], gateDnTx[1]);
        ...
        while (1) {
            ...
            if (usbentsTx == 1) {
#if DBG_27_EPOL
                printf("[ePol] Tx cmd: 0x%.2x, opc: 0x%.2x, dat: 0x%.2x, lastsz: %d, cnt: %d smode: 0x%.2x\n", cmd, opc, dat, lastsz, cntTx, smode);
#endif
                if ((cmd == 0x11) && (opc == 0x4d)) {
                    ...
                    if (chq != 'J') {
                        printf("[DV] poll status unknown result, ret: %c (0x%.2x) \n", chq, chq);
                    }
                ...
                if (pImage) {
                    ...
                    while (1) {       
#if DBG_27_DV
                        printf("[DV] addrd: 0x%.8x cylcnt: %d:%d\n", addrd, uimCylcnt, datCylcnt);
#endif
                        while (addrd == 0) {
                            //chr = 0;
                            while (1) {
                                ...
                                if (endf) {
                                    //printf("[DV] conti stop endf: (%s) seqtx:%d puimCnTH: 0x%.8x \n", endf, seqtx, puimCnTH);
                                    if (!puimCnTH) {
                                        if (endm) {
                                            addrd = endm;
                                    ...
                                    if (puimGet) {
                                        if ((puimGet->uimCount == 0) && ((opc == 0x0a) || (opc == 0x05))) {
                                            ...
                                            if (idlet > 3000) {
                                                ...
                                                waitCylen = ix;
                                                printf("[DV] wait page size: %d timeout\n", waitCylen);
                                ...
                                if (chq == 0xff) {
                                    if (chr) {
                                        ...
                                        if (pipRet < 0) {
                                            printf("[DV]  pipe(%d) put chr: %d ret: %d act 0x80\n", piptx[1], chr, pipRet);
                                            goto end;
                                        }
                                    } else {
                                        printf("[DV] warnning!!! chr should not be 0 !! \n");
                                    }
                                }
                                else if (chq == 0xbf) {
                                    ...
                                    //printf("[DV] chq: 0x%.2x chr: 0x%.2x pipe%d -2 \n", chq, chr, piprx[0]);
                                        
                                    if ((chq & 0xc0) == 0x40) {
                                        cindexfo[1] = chq;
                                    } else {
                                        printf("[DV]  WARNNING get unknown chq: 0x%.2x \n", chq);
                                        continue;
                                    }
                                    ...
                                    if (!puimCnTH) {
                                        ...
                                        if (!chr) {
                                            ...
                                            pipRet = write(piptx[1], mindexfo, 2);
                                            if (pipRet < 0) {
                                                printf("[DV]  pipe(%d) put chr: %d ret: %d \n", piptx[1], chr, pipRet);
                                                goto end;
                                            }
                                        } else {
                                            printf("[DV] warring!!! puimCnTH == 0, and chr == %d \n", chr);
                                            chr = puimGet->uimIdex;
                                            mindexfo[0] = ((chr >> 6) & 0x3f) | 0xc0;
                                            mindexfo[1] = (chr & 0x3f) | 0x40;
                                    ...
                                    if (puimGet) {
                                        ...
                                        if (puimGet) {
                                            printf("[DV] get puim index: 0x%.2x %d:%d\n", puimGet->uimIdex, puimGet->uimGetCnt, puimGet->uimCount);
                                            ...
                                            if (!chr) {
                                                ...
                                                printf("[DV] should not be here chr: (%d) \n", chr);
                                            }
                                        } else {
                                            printf("[DV] wait for more data \n");
                                        }
                                    }
                                    ...
                                    if (!puimGet) {
                                        ...
                                        //printf("[DV] chq: 0x%.2x chr: 0x%.2x pipe%d -3 \n", chq, chr, piprx[0]);
                                        
                                        if ((chq & 0xc0) == 0x40) {
                                            cindexfo[1] = chq;
                                        } else {
                                            printf("[DV]  WARNNING get unknown chq: 0x%.2x - 3\n", chq);
                                            continue;
                                        }
                                    ...
                                    if (chq == 0x7f) {
                                        ...
                                        //printf("[DV] 0x%.2x:0x%.2x:0x%.2x:0x%.2x = lastlen: %d\n", cinfo[2], cinfo[3], cinfo[4], cinfo[5], lastCylen);
                                        ...
                                        waitCylen = ix;
                                        printf("[DV] wait page size: %d, rest page: %d \n", waitCylen, pagerst);
                            ...
                            //printf("[DV] cycle count = %d chr: 0x%.2x\n", uimCylcnt, chr);
                            //usleep(50000);
                    ...
                    if ((che == 'E') && (addrd == 0)) {
                        ...
                        printf("[DV] end time %llu ms start time %llu ms diff: %d \n", time_get_ms(&tend), time_get_ms(&tstart), usCost);                        
                    ...
                    */
                    ...
                    while (1) {
                        ...
                        if (pipRet < 0) {
                            ...
                            //printf("[DV] pipe%d stop machine reg: %c(0x%.2x) \n", pipeRx[0], chq, chq);
        ...
        while(1) {
            ...
            if ((ix % 4) == 0) {
                printf("\n%d: ", ix);
                    ...
                    if (fsmeta) {
                        printf("[DV] find save meta [%s] succeed!!! \n", ptfilepath);
                    ...
                    if ((ptrecv[0] == 0x55) && (ptrecv[1] == 0x53) && (ptrecv[2] == 0x42)) {
                        ...
                        opc = ptrecv[16];
                        dat = ptrecv[17];
                        printf("[DVF] usb get cmd: 0x%.2x opc: 0x%.2x, dat: 0x%.2x smode: 0x%.2x\n",cmd, opc, dat, smode);      
                        if (cmd == 0x11) {      
                                ...
                                if ((opc == 0x4c) && (dat == 0x01)) {                     
                                    ...
                                    printf("\n[DVF] Error!!! unknown usb opc, cmd: 0x%.2x opc: 0x%.2x, dat: 0x%.2x smode: 0x%.2x \n",cmd, opc, dat, smode);      
                                    continue;
                                }
                            ...
                            if ((opc == 0x04) && (dat == 0x85)) {                                    
                                ...
                                if ((puimGet) && ((opc == 0x05) || (opc == 0x0a))) {
                                    ...
                                    printf("[DV] puimGet is null get next \n");
                                }
                                acusz = 0;
                                break;
                            }    
